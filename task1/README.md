...
- The same 224 × 224 RGB image is used before each model's normalization. ResNet-50 uses `IMAGENET1K_V2`; ViT-B/16 uses `IMAGENET1K_V1`; CLIP ViT-B/32 uses the OpenAI pretrained weights with the matching QuickGELU activation. Backbones remain frozen. CLIP zero-shot uses exactly `a photo of a {class}.` and the model's learned logit scale.
